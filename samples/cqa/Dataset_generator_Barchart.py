...
def GenerateOneBarChart(height, width):
    
    min_num_obj = 3
    max_num_obj = 6
    num=np.random.randint(min_num_obj, max_num_obj + 1)
    colors = np.random.uniform(0.0, 0.9,size = (config.max_obj_num,3))
    image = np.ones(shape=(height, width, 3))
    subImages = [np.ones(shape=(height,width,3)) for i in range(config.max_obj_num)]
    ...
    for i in range(num):
        ...
        sx = ex + spaceWidth
        bar_name = 'bar_{}'.format(num)
        bars.append((bar_name, colors, (sx, sy, ex, ey)))

    # add noise
    noises = np.random.uniform(0, 0.05, (height, width,3))
    image = image + noises
    _min = 0.0
    _max = image.max()
    image -= _min
    ...
    return image, subImages, heights, bars

def GenerateBarData(height, width):
